...
// Subscribe to pending members (status === "pending")
export function subscribeToPendingMembers(callback) {
  const q = query(collection(db, PROFILES_COLLECTION), where("status", "==", "pending"));
  return onSnapshot(q, (snapshot) => {
    const pending = snapshot.docs.map(d => ({ uid: d.id, ...d.data() }));
    callback(pending);
  });
}
