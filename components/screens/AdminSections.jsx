          ...
          {stewardsData.map(s => (
            <div key={s.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 0", borderBottom: "1px solid var(--seam)" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 4, flexShrink: 0 }}>
                ...
                <button onClick={() => { setConfirmModal({ title: `Remove ${s.name}?`, message: `${s.name} will be removed from DWA Contacts.`, danger: true, confirmLabel: "YES, REMOVE", onConfirm: () => { const removed = s; setStewardsData(prev => { const updated = prev.filter(x => x.id !== s.id); saveStewards(updated); return updated; }); setToastMsg({ message: `${s.name} removed`, onUndo: () => { setStewardsData(prev => { const restored = [...prev, removed]; saveStewards(restored); return restored; }); } }); } }); }} style={{ ...f(11, 700), color: "var(--red)", background: "none", border: "1px solid rgba(192,57,43,0.3)", borderRadius: 6, padding: "5px 8px", cursor: "pointer" }}>DEL</button>
              </div>
            </div>
          ))}
        </div>
          ...
          {allApprovedUsers.filter(u => { const q = userAdminSearch.toLowerCase(); return !q || u.name?.toLowerCase().includes(q) || u.email?.toLowerCase().includes(q); }).map(u => {
            ...
            <div key={u.uid} style={{ ...card({ padding: "14px", borderLeft: isBanned ? "3px solid var(--red)" : undefined }), ...col(12) }}>
              ...
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                ...
                <button onClick={() => { setConfirmModal({ title: `Demote ${u.name} to Member?`, message: "They will lose steward/officer privileges.", danger: true, confirmLabel: "YES, DEMOTE", onConfirm: async () => { await updateUserRole(u.uid, "member"); setAdminEmails(prev => prev.filter(e => e !== u.email)); setToastMsg({ message: `${u.name} demoted to Member` }); } }); }} style={{ padding: "7px 10px", background: u.role === "member" || !u.role ? "rgba(201,146,42,0.15)" : "rgba(255,255,255,0.03)", border: u.role === "member" || !u.role ? "1px solid rgba(201,146,42,0.3)" : "1px solid var(--seam)", borderRadius: 6, color: u.role === "member" || !u.role ? "var(--gold)" : "var(--text3)", ...f(10, 700), cursor: "pointer" }}>MEMBER</button>
            ...
            {(() => {
              ...
              return rows.map(r => (
                <div key={r.key} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 0", borderBottom: "1px solid var(--seam)" }}>
                  ...
                  {r.role !== "super" && (
                    <button onClick={() => { setConfirmModal({ title: `Demote ${r.name}?`, message: "They will no longer have officer privileges.", danger: true, confirmLabel: "YES, DEMOTE", onConfirm: async () => {
                      if (r.uid) {
                ...
                <>
                  ...
                  {rows.map(s => (
                    <div key={s.key} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 0", borderBottom: "1px solid var(--seam)" }}>
                      ...
                      {s.source === "roster" ? (
                        <button onClick={() => { setConfirmModal({ title: `Remove ${s.name} from roster?`, message: `${s.name} will be removed from the public stewards roster.`, danger: true, confirmLabel: "YES, REMOVE", onConfirm: () => { const removed = s.raw; setStewardsData(prev => { const updated = prev.filter(x => x.id !== s.id); saveStewards(updated); return updated; }); setToastMsg({ message: `${s.name} removed from roster`, onUndo: () => { setStewardsData(prev => { const restored = [...prev, removed]; saveStewards(restored); return restored; }); } }); } }); }} style={{ ...f(11, 700), color: "var(--red)", background: "none", border: "1px solid rgba(192,57,43,0.3)", borderRadius: 6, padding: "5px 8px", cursor: "pointer", flexShrink: 0 }}>REMOVE</button>
                      ) : (
                        <button onClick={() => { setConfirmModal({ title: `Demote ${s.name}?`, message: "They will lose steward privileges.", danger: true, confirmLabel: "YES, DEMOTE", onConfirm: async () => { try { await updateUserRole(s.uid, "member"); setToastMsg({ message: `${s.name} demoted to member` }); } catch(e) { setToastMsg({ message: "Failed to demote." }); } } }); }} style={{ ...f(11, 700), color: "var(--red)", background: "none", border: "1px solid rgba(192,57,43,0.3)", borderRadius: 6, padding: "5px 8px", cursor: "pointer", flexShrink: 0 }}>DEMOTE</button>
                      )}
                    </div>
                  ))}
                </>
